...
export function OrderFilters({ filters, onFiltersChange, orders }: OrderFiltersProps) {
  ...
  const hasActiveFilters = filters.platform !== 'all' || filters.status !== 'all' || filters.dateRange !== 'all'
          ...
          {filters.dateRange !== 'all' && (
            <Badge variant="secondary" className="flex items-center space-x-1">
              ...
              <button
                ...
                className="ml-1 hover:text-red-500"
                title={`Remove date filter: ${dateRangeOptions.find(d => d.value === filters.dateRange)?.label}`}
              >
                <X className="w-3 h-3" />
